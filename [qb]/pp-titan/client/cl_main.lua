...
function CheckStartRob()
  QBCore.Functions.TriggerCallback('pp-titan:server:checkTime', function(time)
      if time then
          TacoTruckBlip = addBlip(Config['Rob']['TruckSpawnCfg'], 585, 1, 'Localização do objetivo')
...
function SpawnGuardsandTruck()
  ...
  TriggerServerEvent('pp-titan:server:policeAlert', Config['Rob']['TruckSpawnCfg'])
  CheckInV = true
  CheckInVehicle()
  Delivery()
    ...
    if(distance < 6) then
      ...
      if IsControlJustPressed(0, 38) then
        if GetVehicleNumberPlateText(vehicle) == "SINFTUCK" then
          ...
          DeletePed(guardPed)
          TriggerServerEvent('pp-titan:server:rewardItem', deliveryLocation.sellprice, deliveryLocation.item, deliveryLocation.itemamount)
...
RegisterNetEvent('pp-titan:client:policeAlert')
AddEventHandler('pp-titan:client:policeAlert', function(targetCoords)
    local P_0 = 250
