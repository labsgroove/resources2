...
QBCore.Functions.CreateCallback('pp-titan:server:checkTime', function(source, cb)
    local src = source
...
RegisterServerEvent('pp-titan:server:rewardItem')
AddEventHandler('pp-titan:server:rewardItem', function(sellprice , item, itemamount)
    local src = source
    local player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('pp-titan:server:policeAlert')
AddEventHandler('pp-titan:server:policeAlert', function(coords)
    local players = QBCore.Functions.GetPlayers()
    for i = 1, #players do
        local player = QBCore.Functions.GetPlayer(players[i])
        if player.PlayerData.job.name == Config['Rob']['Policejobname'] then
            TriggerClientEvent('pp-titan:client:policeAlert', players[i], coords)
        end
    end
end)