...
QBCore.Functions.CreateCallback('pp-helizone:server:checkTime', function(source, cb)
    local src = source
...
RegisterServerEvent('pp-helizone:server:rewardItem')
AddEventHandler('pp-helizone:server:rewardItem', function(sellprice , item, itemamount)
    local src = source
    local player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('pp-helizone:server:policeAlert')
AddEventHandler('pp-helizone:server:policeAlert', function(coords)
    local players = QBCore.Functions.GetPlayers()
    for i = 1, #players do
        local player = QBCore.Functions.GetPlayer(players[i])
        if player.PlayerData.job.name == Config['Rob']['Policejobname'] then
            TriggerClientEvent('pp-helizone:client:policeAlert', players[i], coords)
        end
    end
end)