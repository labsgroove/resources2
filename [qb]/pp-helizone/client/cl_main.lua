...
Citizen.CreateThread(function()
  while true do
      ...
      if dist <= 5.0 then
          sleep = 1
          if dist <= 3.0 then
              ShowHelpNotification('Aperte ~INPUT_CONTEXT~ para aceitar o trabalho')
...
function CheckStartRob()
  QBCore.Functions.TriggerCallback('pp-helizone:server:checkTime', function(time)
      if time then
          TacoTruckBlip = addBlip(Config['Rob']['TruckSpawnCfg'], 574, 1, 'Localização do objetivo')
          QBCore.Functions.Notify('O trabalho começou. veja a localização no mapa')
          SpawnGuardsandTruck()
...
function SpawnGuardsandTruck()
  ...
  TriggerServerEvent('pp-helizone:server:policeAlert', Config['Rob']['TruckSpawnCfg'])
  CheckInV = true
  CheckInVehicle()
  Delivery()
...
function CheckInVehicle()
  while CheckInV do
    ...
    if GetVehicleNumberPlateText(vehicle) == "SINFTUCK" then
      DeliveryBlip = addBlip(deliveryLocation.vector, 162, 5, 'Local de entrega')
    ...
    if(distance < 6) then
      ...
      if IsControlJustPressed(0, 38) then
        if GetVehicleNumberPlateText(vehicle) == "SINFTUCK" then
          ...
          DeletePed(guardPed)
          TriggerServerEvent('pp-helizone:server:rewardItem', deliveryLocation.sellprice, deliveryLocation.item, deliveryLocation.itemamount)
          QBCore.Functions.Notify('Entregue por $'..deliveryLocation.sellprice, 'Parabens!')
          QBCore.Functions.Notify('Você recebeu' .. ' '..deliveryLocation.item, 'Sucesso!')
          QBCore.Functions.Notify(deliveryLocation.itemamount.. "Items", "$$$")
          CheckInVDelivery = false
        else
          QBCore.Functions.Notify('veiculo errado')
        end
      end
    end
...
RegisterNetEvent('pp-helizone:client:policeAlert')
AddEventHandler('pp-helizone:client:policeAlert', function(targetCoords)
    local P_0 = 250
