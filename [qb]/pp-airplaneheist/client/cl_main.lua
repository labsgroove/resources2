...
Citizen.CreateThread(function()
  while true do
      ...
      if dist <= 5.0 then
          sleep = 1
          if dist <= 3.0 then
              ShowHelpNotification('Aperte ~INPUT_CONTEXT~ Pra começar a missão')
...
function CheckStartRob()
  QBCore.Functions.TriggerCallback('pp-airplaneheist:server:checkTime', function(time)
      if time then
          TacoTruckBlip = addBlip(Config['Rob']['TruckSpawnCfg'], 423, 1, 'Local')
          QBCore.Functions.Notify('A missão começou, dirija-se ao local marcado no mapa.', 'success')
          SpawnGuardsandTruck()
...
function SpawnGuardsandTruck()
  ...
  TriggerServerEvent('pp-airplaneheist:server:policeAlert', Config['Rob']['TruckSpawnCfg'])
  CheckInV = true
  CheckInVehicle()
  Delivery()
...
function CheckInVehicle()
  while CheckInV do
    ...
    if GetVehicleNumberPlateText(vehicle) == "SINFTUCK" then
      DeliveryBlip = addBlip(deliveryLocation.vector, 162, 5, 'Entregar')
...
function Delivery()
  while CheckInVDelivery do
    ...
    if(distance < 6) then
      sleep = 1
      ShowHelpNotification('Aperte ~INPUT_CONTEXT~ para entregar')
      if IsControlJustPressed(0, 38) then
        if GetVehicleNumberPlateText(vehicle) == "SINFTUCK" then
          RemoveBlip(DeliveryBlip)
          DeleteEntity(Tacotruck)
          DeletePed(guardPed)
          TriggerServerEvent('pp-airplaneheist:server:rewardItem', deliveryLocation.sellprice, deliveryLocation.item, deliveryLocation.itemamount)
          QBCore.Functions.Notify('Vendido por $'..deliveryLocation.sellprice, 'success')
          QBCore.Functions.Notify('Você recebeu ' .. ' '..deliveryLocation.item, 'success')
          QBCore.Functions.Notify(deliveryLocation.itemamount.. "Itens", "success")
          CheckInVDelivery = false
        else
          QBCore.Functions.Notify('Veiculo errado!', 'error')
        end
      end
    end
...
RegisterNetEvent('pp-airplaneheist:client:policeAlert')
AddEventHandler('pp-airplaneheist:client:policeAlert', function(targetCoords)
    local P_0 = 250
    local P_1 = AddBlipForRadius(targetCoords.x, targetCoords.y, targetCoords.z, 50.0)

    PlaySound(-1, "Event_Start_Text", "GTAO_FM_Events_Soundset", 0, 0, 1)

    TriggerServerEvent('police:server:policeAlert', 'Tem alguma merda rolando')
