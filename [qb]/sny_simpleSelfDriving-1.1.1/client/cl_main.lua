...
client.functions.setTrafficLightsGreen = function()
    local playerCoords = GetEntityCoords(client.playerPed)
    local trafficLights = GetClosestObjectOfType(playerCoords, 20.0, GetHashKey("prop_traffic_01a"), false, false, false)
    if trafficLights ~= 0 then
        SetTrafficLightState(trafficLights, 0)
    end
end
	...
	while true do
        ...
        elseif client.isInVehicle then
            if client.isDriving then
                ...
                client.functions.setTrafficLightsGreen()
            end
        ...
        if client.isDriving then
            if client.target then
                ...
                elseif targetDistance < 500 then
                    client.functions.setNextSegment()
                end
            end
        end
    end
                ...
                if (config.restrictVehicles and client.functions.isVehicleAllowed(vehicleModel)) or (not config.restrictVehicles) then
                    if client.isDriving then
                        ...
                        if IsWaypointActive() then
                            ...
                            client.functions.setRouteSegments(waypointCoords)
                            client.functions.startSelfDriving(client.playerPed, playerVehicle, client.routeSegments[1], true)
                        else
                            client.functions.showNotification('waypoint_not_active', 'error')
                            client.functions.playSound('error')
                        end
