...
client.functions.startSelfDriving = function(playerPed, playerVehicle, waypointCoords, playSound)
    if DoesEntityExist(playerPed) and DoesEntityExist(playerVehicle) then
        ...
        TaskVehicleDriveToCoordLongrange(playerPed, client.currentVehicle, client.target.x, client.target.y, client.target.z, config.drivingSpeed, client.drivingStyle, config.drivingDistanceStop)
        if locked ~= 2 then
...
client.functions.setRouteSegments = function(waypointCoords)
    client.routeSegments = { waypointCoords }
end

client.functions.setNextSegment = function()
    -- Placeholder for future route segment logic
end
