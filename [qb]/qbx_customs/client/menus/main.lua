...
local function repair()
    ...
    if success then
        if QBCore and QBCore.Functions and QBCore.Functions.Notify then
            QBCore.Functions.Notify(locale('notifications.success.repaired'), 'success')
        else
            lib.notify({title = locale('notifications.success.repaired'), type = 'success'})
        end
        if utils and type(utils.PlayAudio) == 'function' then
            utils.PlayAudio({ audioName = 'PICK_UP', audioRef = 'HUD_FRONTEND_DEFAULT_SOUNDSET' })
        else
        ...
        if QBCore and QBCore.Functions and QBCore.Functions.Notify then
            QBCore.Functions.Notify(locale('notifications.error.money'), 'error')
        else
            lib.notify({title = locale('notifications.error.money'), type = 'error'})
        end
    end
