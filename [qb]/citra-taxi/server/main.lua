if Config.framework == 'qb' then
    ...
    -- Commands
    QBCore.Commands.Add('taxi', 'Chamar taxi', {}, false, function(source, _, _)
        TriggerClientEvent('citra-taxi:client:callOrCancelTaxi', source)
    end, 'user')

    QBCore.Commands.Add('taxigo', 'Iniciar corrida', {}, false, function(source, _, _)
        TriggerClientEvent('citra-taxi:client:setDestination', source)
    end, 'user')

    QBCore.Commands.Add('taxifast', 'Acelerar', {}, false, function(source, _, _)
        TriggerClientEvent('citra-taxi:client:speedUp', source)
    end, 'user')

    QBCore.Commands.Add('taxislow', 'Diminuir', {}, false, function(source, _, _)
    ...
    RegisterNetEvent('citra-taxi:server:payFare', function(time)
        ...
        if fare > 0 then
            ...
            if Player.Functions.RemoveMoney('cash', fare, 'Preço da corrida') then
