...
RegisterNUICallback('takeOutVehicle', function(data, cb)
    if data.remote then
        -- Spawn remoto: spawna na frente do jogador
        ...
        end, data.plate, data.vehicle, spawnCoords, true)
    else
        -- Spawn padrão: usa o evento
...
RegisterCommand('gar', function()
    ...
    if veh ~= 0 then
        -- Guarda o veículo remotamente
        local plate = QBCore.Functions.GetPlate(veh)
        -- Procura o tipo e index da garagem do veículo, ou usa 'public' como padrão
        local data = {
            type = 'public',
            indexgarage = 1, -- pode ajustar para buscar a garagem correta se necessário
            category = Config.VehicleClass[GetVehicleClass(veh)] or Config.VehicleClass['car']
        }
        DepositVehicle(veh, data)
    else
        -- Abre o menu de garagem remota
        local playerCoords = GetEntityCoords(ped)
        local fakeGarage = {
            indexgarage = 0,
            type = 'public',
            category = Config.VehicleClass['car'],
            label = 'Garagem Remota',
            spawnPoint = { vector4(playerCoords.x + 1, playerCoords.y - 2, playerCoords.z, GetEntityHeading(ped)) }
        }
        QBCore.Functions.TriggerCallback('qb-garages:server:GetGarageVehicles', function(result)
            ...
            for _, v in pairs(result) do
                ...
                formattedVehicles[#formattedVehicles + 1] = {
                    ...
                    depotPrice = v.depotprice or 0,
                    balance = v.balance or 0,
                    stats = { fuel = v.fuel, engine = v.engine, body = v.body }
                }
            end
            SetNuiFocus(true, true)
            SendNUIMessage({
                action = 'VehicleList',
                garageLabel = fakeGarage.label,
                vehicles = formattedVehicles,
                remote = true -- flag para identificar chamada remota
            })
        end, 0, 'public', Config.VehicleClass['car'])
    end
end)
