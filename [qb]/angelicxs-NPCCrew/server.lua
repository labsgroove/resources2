...
-- Evento para atualizar a gangue do jogador
RegisterNetEvent('angelicxs-NPCCrew:Server:SetPlayerGang', function(crew)
    local src = source
    if Config.UseESX then
        local xPlayer = ESX.GetPlayerFromId(src)
        if xPlayer then
            xPlayer.setJob(crew, 0) -- Atualiza o trabalho do jogador para a gangue da crew
            TriggerClientEvent('angelicxs-NPCCrew:Notify', src, Config.Lang['gang_assigned'], Config.LangType['success'])
        end
    elseif Config.UseQBCore then
        local player = QBCore.Functions.GetPlayer(src)
        if player then
            player.Functions.SetGang(crew) -- Atualiza a gangue do jogador para a gangue da crew
            TriggerClientEvent('angelicxs-NPCCrew:Notify', src, Config.Lang['gang_assigned'], Config.LangType['success'])
        end
    end
end)
