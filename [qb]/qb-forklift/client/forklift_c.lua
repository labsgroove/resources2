...
local kartonKoord1 = {x = 129.7338, y = -3185.2468, z = 5.8576}
local kartonKoord2 = {x = 139.6580, y = -3192.0359, z = 5.8576}
local kartonKoord3 = {x = 147.8296, y = -3184.9006, z = 5.8576}
local kartonKoord4 = {x = 139.7889, y = -3182.9097, z = 5.8576}
local kartonKoord5 = {x = 145.5079, y = -3189.4468, z = 5.8576}
...
AddEventHandler('qb-forklift:rampanpcdawaj',function(a)
    ...
    if a=='1'then
        if zrespione==true then
			...
			QBCore.Functions.Notify('Complete esse trabalho primeiro!', 'success')
            return
        end
        ...
        if b==1 then
            ...
            QBCore.Functions.Notify('O motorista está aguardando a carga de cremes.', 'success')
            ZrespPaczuszke('deski')
        elseif b==2 then
            naklejkaAuto=4
            
            QBCore.Functions.Notify('O motorista aguarda a carga de colonias.', 'success')
            ZrespPaczuszke('lody')
        elseif b==3 then
            naklejkaAuto=6
           
            QBCore.Functions.Notify('O motorista espera o caregamento de shampoos.', 'success')
            ZrespPaczuszke('leki')
        elseif b==4 then
            naklejkaAuto=2
          
            QBCore.Functions.Notify('Tem um caminhão pra ser carregado com amostras.', 'success')
            ZrespPaczuszke('napoje')
        elseif b==5 then
            naklejkaAuto=1
            
            QBCore.Functions.Notify('Esse palet é smart box.', 'success')
            ZrespPaczuszke('kawa')
        end
        ...
        QBCore.Functions.Notify('O caminhão está vindo do retro patio.', 'Nota faturada')
        zrespione=true
        silnikOn=true
        Citizen.Wait(900)
        while silnikOn do
        ...
        QBCore.Functions.Notify('O motorista estacionou e abriu o baú', 'success')
        ...
        QBCore.Functions.Notify('Caminhão carregado e partindo.', 'success')
        
        QBCore.Functions.Notify('Você carregou em '..czasikDostaw..' segundos.', 'success')
        if czasikDostaw<41 then
            
            QBCore.Functions.Notify('Bonus '..premia1 ..'para essa carga', 'success')
            TriggerServerEvent("qb-forklift:wykonanieMisji",premia1)
            Citizen.Wait(200)
        elseif czasikDostaw>=41 and czasikDostaw<=55 then
            
            QBCore.Functions.Notify('Bonus '..premia2 ..' para essa carga', 'success')
            TriggerServerEvent("qb-forklift:wykonanieMisji",premia2)
            Citizen.Wait(200)
        elseif czasikDostaw>=56 and czasikDostaw<=65 then
            
            QBCore.Functions.Notify('Bonus '..premia3 ..' para essa carga', 'success')
            TriggerServerEvent("qb-forklift:wykonanieMisji",premia3)
            Citizen.Wait(200)
        elseif czasikDostaw>65 then
           
            QBCore.Functions.Notify('Demorou muito, sem bonus dessa vez', 'success')
        end
        czasikDostaw=0
        dostawaTimer=false
        ...
        QBCore.Functions.Notify('Você pode pegar outra ordem de carga', 'success')
    elseif a=='2'then
        if zrespione==true then
           
            QBCore.Functions.Notify('Complete o ultimo serviço!', 'success')
            return
        end
        local b=math.random(1,5)
        if b==1 then
        ...
        QBCore.Functions.Notify('O caminhão está vindo do retro patio.', 'success')
        zrespione=true
        silnikOn=true
        Citizen.Wait(900)
        while silnikOn do
        ...
        QBCore.Functions.Notify('Você carregou em '..czasikDostaw..' segundos.', 'successo')
        if czasikDostaw<61 then
           
            QBCore.Functions.Notify('Bonus '..premia1 ..'$ para essa carga', 'successo')
            TriggerServerEvent("qb-forklift:wykonanieMisji",premia1)
            Citizen.Wait(200)
        elseif czasikDostaw>=61 and czasikDostaw<=75 then
           
            QBCore.Functions.Notify('Bonus '..premia2 ..'$ para essa carga', 'successo')
            TriggerServerEvent("qb-forklift:wykonanieMisji",premia2)
            Citizen.Wait(200)
        elseif
            czasikDostaw>=76 and czasikDostaw<=85 then
           
            QBCore.Functions.Notify('Bonus '..premia3 ..'$ para essa carga', 'successo')
            TriggerServerEvent("qb-forklift:wykonanieMisji",premia3)
            Citizen.Wait(200)
        elseif
            czasikDostaw>85
...
Citizen.CreateThread(function()
    while true do
		...
		czasikDostaw = czasikDostaw +1
            ...
            QBCore.Functions.Notify('O técnico mandou você passar no RH', 'success')
			end
		else
		Citizen.Wait(2000)
		end	
...
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(3)
			...
			if distCar <= 1.0 then
				DrawText3D(wezCar.x, wezCar.y, wezCar.z, "[E] Pegue uma emipilhadeira")
			...
			if zlecDist <= 1.0 then
				DrawText3D(zlecenieDist.x, zlecenieDist.y, zlecenieDist.z, "[E] Comece uma carga")
				if IsControlJustPressed(0, Keys['E']) then 
                    ...
                    QBCore.Functions.Notify('O palete está no local indicado, carregue no caminhão com a empilhadeira.', 'successo', 5000)
					--else
                   
                    --QBCore.Functions.Notify('You must take over the hangar', 'success', 5000)
					--end
    ...
    while true do
        Citizen.Wait(3)
			...
			if distCar2 <= 1.0 then
				DrawText3D(wezCar2.x, wezCar2.y, wezCar2.z, "[E] Pegue uma empilhadeira")
				if IsControlJustPressed(0, Keys['E']) then 
				TakeForklift('2')
				end
            end
			...
			if zlecDist2 <= 1.0 then
				DrawText3D(zlecenieDist2.x, zlecenieDist2.y, zlecenieDist2.z, "[E] Comece uma carga")
				if IsControlJustPressed(0, Keys['E']) then 
                    ...
                    QBCore.Functions.Notify('O palete está no local indicado, carregue no caminhão com a empilhadeira.', 'success')
...
function TakeForklift(a)
    if a=='1'then
        if PojazdWyjety==0 then
           ...
           --[[
            ...
            QBCore.Functions.Notify('Your previous vehicle has been deleted, you can retreive a new one', 'success')]]--
        end
    ...
    elseif a=='2'then
        if PojazdWyjety==0 then
            ...
            QBCore.Functions.Notify('Você guardou uma empilhadeira, quando precisar é só pegar outra', 'success')
        end
    end
end
