...
function CheckStartRob()
  QBCore.Functions.TriggerCallback('pp-cayo:server:checkTime', function(time)
      if time then
          TacoTruckBlip = addBlip(Config['Rob']['TruckSpawnCfg'], 226, 1, 'Localização do iate')
...
function SpawnGuardsandTruck()
  ...
  TriggerServerEvent('pp-cayo:server:policeAlert', Config['Rob']['TruckSpawnCfg'])
  CheckInV = true
  CheckInVehicle()
  Delivery()
    ...
    if(distance < 6) then
      ...
      if IsControlJustPressed(0, 38) then
        if GetVehicleNumberPlateText(vehicle) == "SINFTUCK" then
          ...
          DeletePed(guardPed)
          TriggerServerEvent('pp-cayo:server:rewardItem', deliveryLocation.sellprice, deliveryLocation.item, deliveryLocation.itemamount)
...
RegisterNetEvent('pp-cayo:client:policeAlert')
AddEventHandler('pp-cayo:client:policeAlert', function(targetCoords)
    local P_0 = 250
